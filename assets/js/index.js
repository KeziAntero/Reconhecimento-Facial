...
// Função para mapear as expressões para português
const translateExpression = (expression) => {
    ...
    return expressionMap[expression] || expression; // Retorna a tradução ou a própria expressão se não mapeada
}
...
cam.addEventListener('play', async () => {
    ...
    setInterval(async () => {
        ...
        // Atualiza a informação no HTML
        resizedDetections.forEach((detection, index) => {
            const { age, gender, expressions } = detection
            const result = results[index]
            const identification = result.label !== 'unknown' ? result.label : 'Desconhecido'

            // Encontra a expressão mais provável
            const maxExpression = Object.keys(expressions).reduce((a, b) => expressions[a] > expressions[b] ? a : b)

            // Traduz a expressão para português
            ...
            document.getElementById('gender').innerText = gender === 'male' ? 'Masculino' : 'Feminino'
